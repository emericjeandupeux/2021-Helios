...
void display_son(uint8_t db)
{
	if (db < 80)
	{
		u8g2_DrawXBM(&u8g2, 3, 5, son60_width, son60_height,
				(uint8_t*) son60_bits);
	}
	if (db >= 80 && db < 120)
	{
		u8g2_DrawXBM(&u8g2, 3, 5, son80_width, son80_height,
				(uint8_t*) son80_bits);
	}
	if (db >= 120)
	{
		u8g2_DrawXBM(&u8g2, 3, 5, son120_width, son120_height,
				(uint8_t*) son120_bits);
	}
...
void display_warning(uint8_t nb)
{
	for (int i = 0; i < nb; i++)
	{
		...
		do
		{
			u8g2_DrawXBM(&u8g2, 32, 0, warning_width, warning_height,
					(uint8_t*) warning_bits);
		} while (u8g2_NextPage(&u8g2));
		HAL_Delay(500);
		u8g2_ClearDisplay(&u8g2);
		HAL_Delay(200);
...
void display_Wifi(int TabGen[], char name[])
{
	if (WIFI == 1)
	{
		if (name[0] != 'N' && name[1] != 'o' && name[2] != 'N' && name[3] != 'a'
				&& name[4] != 'm' && name[5] != 'e')
		{
			char str_display[18];
			u8g2_SetFont(&u8g2, u8g2_font_5x8_tr);
...
void NetworkStatus(int TabGen[])
{
	...
	if (c1.com_sigfox == 1 && TabGen[SIGFOX_RSSI] != 0)
	{
		char str_display[18];
		u8g2_SetFont(&u8g2, u8g2_font_5x8_tr);
	...
	if (c1.com_LoRa == 1 && TabGen[LORAWAN_RSSI] != 0)
	{
		char str_display[18];
		u8g2_SetFont(&u8g2, u8g2_font_5x8_tr);
...
void display_Chargement(void)
{
	if (DISPLAYCHARGHES == 1)
	{
		for (int i = 0; i < 2; i++)
		{
			...
			do
			{
				u8g2_DrawXBM(&u8g2, 0, 0, hestiam_width, hestiam_height,
						(uint8_t*) hestiam_bits);
			} while (u8g2_NextPage(&u8g2));
			HAL_Delay(50);
			u8g2_FirstPage(&u8g2);
			do
			{
				u8g2_DrawXBM(&u8g2, 0, 0, hestiam1_width, hestiam1_height,
						(uint8_t*) hestiam1_bits);
			} while (u8g2_NextPage(&u8g2));
			HAL_Delay(50);
			u8g2_FirstPage(&u8g2);
			do
			{
				u8g2_DrawXBM(&u8g2, 0, 0, hestiam2_width, hestiam2_height,
						(uint8_t*) hestiam2_bits);
			} while (u8g2_NextPage(&u8g2));
			HAL_Delay(50);
			u8g2_FirstPage(&u8g2);
			do
			{
				u8g2_DrawXBM(&u8g2, 0, 0, hestiam3_width, hestiam3_height,
						(uint8_t*) hestiam3_bits);
			} while (u8g2_NextPage(&u8g2));
			u8g2_FirstPage(&u8g2);
			do
			{
	...
	else
	{
		for (int i = 0; i < 2; i++)
		{
			...
			do
			{
				u8g2_DrawXBM(&u8g2, 0, 0, regaz_width, regaz_height,
						(uint8_t*) regaz_bits);
			} while (u8g2_NextPage(&u8g2));
			HAL_Delay(50);
			u8g2_FirstPage(&u8g2);
			do
			{
				u8g2_DrawXBM(&u8g2, 0, 0, regaz1_width, regaz1_height,
						(uint8_t*) regaz1_bits);
			} while (u8g2_NextPage(&u8g2));
			HAL_Delay(50);
			u8g2_FirstPage(&u8g2);
			do
			{
				u8g2_DrawXBM(&u8g2, 0, 0, regaz2_width, regaz2_height,
						(uint8_t*) regaz2_bits);
			} while (u8g2_NextPage(&u8g2));
			HAL_Delay(50);
			u8g2_FirstPage(&u8g2);
			do
			{
				u8g2_DrawXBM(&u8g2, 0, 0, regaz3_width, regaz3_height,
						(uint8_t*) regaz3_bits);
			} while (u8g2_NextPage(&u8g2));
			u8g2_FirstPage(&u8g2);
			do
...
void display_Print_Sound_Level(uint8_t choice)
{
	...
	case STAGE1:
		do
		{
			u8g2_DrawXBM(&u8g2, 32, 0, stage1_width, stage1_height,
					(uint8_t*) stage1_bits);
		} while (u8g2_NextPage(&u8g2));
		break;
	case STAGE2:
		do
		{
			u8g2_DrawXBM(&u8g2, 32, 0, stage2_width, stage2_height,
					(uint8_t*) stage2_bits);
		} while (u8g2_NextPage(&u8g2));
		break;
	case STAGE3:
		do
		{
			u8g2_DrawXBM(&u8g2, 32, 0, stage3_width, stage3_height,
					(uint8_t*) stage3_bits);
		} while (u8g2_NextPage(&u8g2));
		break;
	case STAGE4:
		do
		{
			u8g2_DrawXBM(&u8g2, 32, 0, stage4_width, stage4_height,
					(uint8_t*) stage4_bits);
		} while (u8g2_NextPage(&u8g2));
		break;
	}
 ...
 */
void display_Print_UART(char *msg, uint8_t xpos, uint8_t ypos,
		uint8_t sizePolice)
{
	u8g2_FirstPage(&u8g2);
	do
